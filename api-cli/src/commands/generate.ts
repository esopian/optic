...
import {ScraperApiClient} from '@useoptic/cogent/dist/src/api-artifact-generators/scraper-api-client'
...
export default class Generate extends Command {
  ...
  async generateArtifacts(artifacts: any[], representation: any, outputPath: string) {
    ...
    reconciler.emit(output, {outputDirectory: outputPath})
  }

  async run() {
    const paths = await getPaths()
    const {artifactId} = this.parse(Generate).args
    const representation = await this.makeCogentApiArtifactGeneratorRepresentation(paths)
    await this.generateArtifacts([
      {artifactId, config: {}}
    ], representation, path.join(paths.outputPath, artifactId))
  }
}
...
class ApiArtifactRegistry implements ICogentComponentRegistry<IApiArtifactGeneratorProps> {
  resolve(id: string): React.ComponentType<IApiArtifactGeneratorProps> | null {
    const r = {
      'javascript-api-client': JavascriptApiClient,
      'scraper-api-client': ScraperApiClient,
    }
    return r[id]
  }
}
